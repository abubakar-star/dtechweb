...
// ================= ACTIVATE UPDATED PACKAGE =================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['activate_package'])) {
    ...
    // Save popup flag for this user
    $stmt = $conn->prepare("
        UPDATE users
        SET package_updated_popup = '1'
        WHERE id = ?
    ");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
<?php else: ?>
    ...
    <span class="px-2 py-1 rounded text-xs bg-gray-200 text-gray-800">
        Updated
    </span>
...
<form method="POST">
    <input type="hidden" name="activate_package">
    <input type="hidden" name="user_id" value="<?= $u['id'] ?>">

    <button
        onclick="return confirm('Activate this package update?')"
