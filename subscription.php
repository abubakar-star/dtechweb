...
function initializeExtraCharges(invoiceNumber, payBtn) {
    fetch("initialize_extra_charges.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded"
        },
        body: "invoice_number=" + encodeURIComponent(invoiceNumber)
    })
    .then(r => r.json())
    .then(data => {
        if (!data.success) {
            showStkError(data.message || "Failed to initialize extra charges");
            resetPayButton(payBtn);
            return;
        }
        ...
        showStkLoading(
            "STK Push Sent",
            "A payment prompt has been sent to your phone for the outstanding charges. Please complete it to continue."
        );

        pollExtraChargeStatus(data.reference, invoiceNumber, payBtn);
    })
    .catch(error => {
        console.error(error);
        showStkError("Extra charges payment error");
        resetPayButton(payBtn);
    });
}

function pollExtraChargeStatus(reference, invoiceNumber, payBtn) {
    let attempts = 0;
    ...
    const interval = setInterval(() => {
        ...
        .then(data => {
            ...
            if (data.status === "completed") {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Outstanding Charges Paid',
                    text: 'Outstanding charges paid successfully. Subscription STK push will be sent after 60 seconds.',
                ...
                const countdownInterval = setInterval(() => {
                    payBtn.innerHTML = `Subscription payment in ${countdown}s`;
                    countdown--;

                    if (countdown < 0) {
                        clearInterval(countdownInterval);

                        payBtn.innerHTML = "Sending Subscription STK...";

                        showStkLoading(
                            "Sending Subscription STK",
                            "A payment prompt is being sent to your phone for the subscription package."
                        );

                        initializeSubscription(invoiceNumber, payBtn);
                    }
                }, 1000);
            }
                ...
                clearInterval(interval);
                Swal.close();
                showStkError(data.failure_reason || "Extra charges payment failed");
                resetPayButton(payBtn);
            }
        })
        .catch(error => {
        ...
        if (attempts >= 60) {
            clearInterval(interval);
            Swal.close();
            showStkError("Extra charges payment timed out");
            resetPayButton(payBtn);
        }
    }, 2000);
}

function initializeSubscription(invoiceNumber, payBtn) {
    showStkLoading(
        "Sending Subscription STK",
        "A payment prompt has been sent to your phone. Complete it to activate or update your package."
    );
    ...
    .then(data => {
        if (!data.success) {
            Swal.close();
            showStkError(data.message || "Subscription initialization failed");
            resetPayButton(payBtn);
            return;
        }
    ...
    .catch(error => {
        console.error(error);
        Swal.close();
        showStkError("Subscription payment error");
        resetPayButton(payBtn);
    });
}
    ...
    const interval = setInterval(() => {
        ...
        .then(data => {
            ...
            if (data.status === "completed") {
                ...
                showStkSuccess(
                    "Payment successful. Your package has been processed.",
                    () => {
            ...
            // PAYMENT FAILED
                ...
                Swal.close();
                showStkError(data.failure_reason || "Payment failed");
        ...
        if (attempts >= 60) {
            ...
            Swal.close();
            showStkError("Payment confirmation timeout. Please refresh later.");
...
const currentPackageId = <?php echo json_encode($userPackageId); ?>;
  ...
  function showStkLoading(title, text = '') {
  Swal.fire({
    title: title,
    html: text,
...
function showStkSuccess(message, callback = null) {
  Swal.fire({
    icon: 'success',
