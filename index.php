...
$userStatus = $user['status'];
$packageUpdatedPopup = (int)($user['package_updated_popup'] ?? 0);
...
// TIMEOUT
  ...
  const createdAt = <?= $createdAt ? '"' . $createdAt . '"' : 'null' ?>;
...
if (isInactive) {
 ...
 if (userStatus === 'queued') {
    ...
    if (createdAt === null) {
        ...
        statusBadge.className =
            "block text-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-700";
...
<?php if ($userStatus === 'queued' && empty($createdAt)): ?>
    ...
    Swal.fire({
        ...
        html: `
            <div style="font-size:15px;line-height:1.7">
                <b>Your payment has been received successfully.</b><br><br>

                Our administrator is currently updating and verifying your new package.<br><br>

                Your new subscription will begin immediately after verification.
            </div>
        `,
...
<?php if ($userStatus === 'active' && !empty($createdAt) && $packageUpdatedPopup === 1): ?>
<script>
document.addEventListener("DOMContentLoaded", function () {
    Swal.fire({
        icon: "success",
        ...
        html: `
            <div style="font-size:15px;line-height:1.7">
                <b>Your package has been updated successfully.</b><br><br>
