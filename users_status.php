...
$result = $conn->query("
    SELECT
        ...
        Expiry,
        status,
        DATEDIFF(Expiry, CURDATE()) AS days_left
    FROM users
    ORDER BY
        CASE
        ...
        END,
        CASE
            WHEN status='active' THEN DATEDIFF(Expiry, CURDATE())
            ELSE 9999
        END ASC,
        Expiry ASC
");
            ...
            <tbody class="text-sm">
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        <td class="p-3">
    <?= htmlspecialchars($row['Expiry']) ?>
</td>

<td class="p-3">
    <?php
    if ($row['days_left'] > 0) {
        echo $row['days_left'] . " day(s)";
    } elseif ($row['days_left'] == 0) {
        echo "<span class='text-orange-600 font-bold'>Expires Today</span>";
    } else {
        echo "<span class='text-red-600 font-bold'>Expired " . abs($row['days_left']) . " day(s) ago</span>";
    }
    ?>
</td>
                            <td class="p-3">
